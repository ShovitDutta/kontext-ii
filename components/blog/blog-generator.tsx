...
type GeneratingState = {
    [key in ContentLength]?: boolean;
};

type GeneratedContentState = {
    [key in ContentLength]?: string;
};
    ...
    const generateContent = async (length: ContentLength) => {
        ...
        try {
            ...
            if (response.headers.get('Content-Type')?.includes('application/json')) {
                // Handle non-streaming response (cached content)
        ...
        return (
            <div className="prose prose-invert mt-4 bg-gray-800 p-4 rounded-lg">
                {content.split('\n').map((line, i) => {
                    ...
                    if (line.startsWith('> ')) {
                        return <blockquote key={i} className="border-l-4 border-gray-500 pl-4 italic my-4">{line.substring(2)}</blockquote>;
                    }
                    if (line.startsWith('- ')) {
        ...
        <div className="space-y-8">
            <div className="flex space-x-4">
                {(Object.keys(ContentLength) as Array<keyof typeof ContentLength>).map((key) => (
                    ...
                    >
                        {generating[ContentLength[key]] && <Loader2 className="w-4 h-4 animate-spin" />}
                ...
                {(Object.keys(ContentLength) as Array<keyof typeof ContentLength>).map((key) => (
                    <div key={key}>
                        {generatedContent[ContentLength[key]] && (
                             <div className="prose prose-invert mt-4 bg-gray-800 p-4 rounded-lg whitespace-pre-wrap">{generatedContent[ContentLength[key]]}</div>
                        )}
                    </div>
                ))}
            </div>
